...
class CrossAttention(nn.Module):
    ...
    def __init__(
        ...
        # Enable memory efficient attention by default for low VRAM
        self._use_memory_efficient_attention_xformers = XFORMERS_AVAILABLE
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        if XFORMERS_AVAILABLE and self._use_memory_efficient_attention_xformers:
            ...
            # Use sliced attention for memory efficiency on low VRAM
            if torch.cuda.is_available():
                available_memory = torch.cuda.get_device_properties(0).total_memory / 1024**3
                if available_memory < 8 and self._slice_size is None:
                    self._slice_size = min(4, self.heads)  # Force slicing for low VRAM
    ...
    def _memory_efficient_attention_split(self, query, key, value, attention_mask):
        ...
        # Reduce batch size for low VRAM devices
        if torch.cuda.is_available():
            available_memory = torch.cuda.get_device_properties(0).total_memory / 1024**3
            if available_memory < 8:
                max_batch_size = 16384  # Smaller batches for low VRAM
            else:
