    ...
    def infer_video_depth(self, frames, target_fps, input_size=518, device='cuda', fp32=False):
        ...
        for frame_id in tqdm(range(0, org_video_len, frame_step)):
            ...
            try:
                ...
                with torch.no_grad():
                    with torch.autocast(device_type=device, enabled=(not fp32), dtype=torch.float16):
                ...
                # Fallback: process with smaller chunks
                torch.cuda.empty_cache()
                
                # Process frames individually if batch fails
                chunk_depths = []
                for i in range(INFER_LEN):
